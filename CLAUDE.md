# Claude Code Project Instructions — color-eyre-kotlin

## Project Overview

This is **color-eyre-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`color-eyre`](https://crates.io/crates/color-eyre).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/color-eyre/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/color-eyre>
